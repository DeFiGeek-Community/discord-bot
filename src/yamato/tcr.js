    ...
    setInterval(async () => {
      try {
        ...
        const totalCollateralETH =
          (allYamatoStates[0] * rateOfEthJpy) / 10 ** 18; //18924045.2508;//allYamatoStates[0] / 10 ** 18;
        const totalSupplyCJPY = allYamatoStates[1] / 10 ** 18;
        const tcr =
          totalSupplyCJPY === 0
            ? 0
            : ((totalCollateralETH / totalSupplyCJPY) * 100).toFixed(2);

        await bot.setNickname(`TCR: ${tcr}%`);

        var redeemablesCandidate =
          await PriorityRegistryV6contract.getRedeemablesCap();
        if (redeemablesCandidate !== 0)
          redeemablesCandidate = redeemablesCandidate / 10 ** 18;

        await yamatoTcrBotClient.user.setActivity(
          `償還候補: ${redeemablesCandidate} CJPY`
        );
      } catch (err) {
        console.error("yamato/tcr.js" + err.name + ": " + err.message);
      }
    }, intervalTime);
  });
