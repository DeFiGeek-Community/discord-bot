  ...
  yamatoCollateralBotClient.on("ready", async () => {
    setInterval(async () => {
      try {
        ...
        await yamatoCollateralBotClient.user.setActivity(
          `総発行: ${totalSupplyCJPY.toLocaleString()} CJPY`
        );
      } catch (err) {
        console.error("yamato/collateral.js" + err.name + ": " + err.message);
      }
    }, intervalTime);
  });

  yamatoCollateralBotClient.login(
    process.env.DISCORD_YAMATO_COLLATERAL_BOT_TOKEN
  );
};
